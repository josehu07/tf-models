...
def rand_num(max_lim: int):
    # this function is to ensure we don't have too many duplication record
    # which might be compressed when convered into TFRecord
    return random.randint(1, max_lim) + random.random()
...
class RandomCocoRecord():
    # create fake Coco dataset
    def __init__(self, anno_id, img_id) -> None:
        self.annotations = {
            'area': rand_num(20191),
            'bbox': [rand_num(311), rand_num(220), rand_num(40), rand_num(30)],
            'category_id': random.randint(1, 90),
            'id': anno_id,
            'image_id': img_id,
            'iscrowd': 0,
            'segmentation': [[rand_num(400) for _ in range(35)]]
        }
        self.images = {
            'coco_url': f'http://images.cocodataset.org/train2017/{img_id:012}.jpg',
            'date_captured': f'2013-11-{random.randint(1, 28)} 14:45:46',
            'file_name': f'{img_id:012}.jpg',
            'flickr_url': f'http://farm4.staticflickr.com/{random.randint(1, 2333)}/9313010404_1d770a8fdd_z.jpg',
            'height': 480,
            'id': img_id,
            'license': 1,
            'width': 640
...
if __name__ == "__main__":
    records = [RandomCocoRecord(img_id * 2, img_id) for img_id in range(1, int(sys.argv[1]))]
    data = {}
    data['annotations'] = [r.annotations for r in records]
    data['categories'] = get_categories()
    data['images'] = [r.images for r in records]
    data['info'] = {'contributor': 'COCO Consortium',
                    'date_created': '2017/09/01',
                    'description': 'COCO 2017 Dataset',
                    'url': 'http://cocodataset.org',
                    'version': '1.0',
                    'year': 2017}
    data['licenses'] = [r.licenses for r in records]
    with open("retinanet_data_gen/annotations/instances_val2017.json", "w") as f:
        json.dump(data, f)
